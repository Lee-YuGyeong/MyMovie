...
public class Fragment_home_3 extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        textView.setText("3. " + arrayList.get(2).getTitle());
        textView2.setText("예매율  " + arrayList.get(2).getReservation_rate() + "% | " + arrayList.get(2).getGrade() + "세 관람가 | " + arrayList.get(2).getDateValue() + " 개봉");

        String url = arrayList.get(2).getImage();
        Glide.with(getActivity()).load(url).into(imageView);

        id = arrayList.get(2).getId();
